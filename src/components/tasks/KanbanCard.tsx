...
export function KanbanCard({
  ...
  return (
    <div
      ...
      className={`group bg-slate-900 border border-slate-800 rounded-lg p-3 transition-all cursor-pointer ${
        isDragging ? 'opacity-50 scale-95' : 'opacity-100'
      } ${readOnly ? '' : 'hover:border-indigo-700 hover:shadow-sm'}`}
    >
      ...
      <div className="flex items-start gap-2 mb-2">
        {!readOnly && (
          <GripVertical
            size={13}
            className="text-slate-600 flex-shrink-0 mt-0.5 group-hover:text-slate-500 transition-colors cursor-grab active:cursor-grabbing"
            aria-hidden
          />
        )}
      ...
      {/* Source label */}
      <p className="text-xs text-slate-400 truncate mb-2 pl-1">
        {task.source_label}
      </p>
      ...
      {/* Due + progress */}
      <div className="flex items-center gap-1.5 text-xs text-slate-400 mb-2 pl-1">
      ...
      {!readOnly && (
        <div className="mt-3 pt-3 border-t border-slate-800">
          ...
          <select
            ...
            className="w-full text-xs px-2 py-1.5 bg-slate-800 border border-slate-700 rounded text-slate-200 focus:outline-none focus:border-indigo-500 transition"
          >
