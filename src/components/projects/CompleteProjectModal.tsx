      ...
      {hasUnfinished ? (
        <>
          <p className="text-sm text-slate-600 dark:text-slate-300 mb-4">
            ...
            <span className="text-slate-900 dark:text-white font-medium">Completed Projects / History</span>, but
            unfinished tasks will remain visible in project history.
          </p>

          {/* List unfinished tasks */}
          <div className="bg-slate-100 dark:bg-slate-800 rounded-lg p-3 mb-5 space-y-1.5">
            ...
            {unfinishedTasks.map((t) => (
              <div key={t.id} className="flex items-center gap-2">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-300 truncate">{t.title}</p>
              </div>
            ))}
          </div>
        ...
        <>
          <p className="text-sm text-slate-600 dark:text-slate-300 mb-6">
            ...
            <span className="text-slate-900 dark:text-white font-medium">Completed Projects / History</span>.
          </p>
          <div className="flex justify-end gap-3">
            <Button variant="secondary" onClick={onClose}>Cancel</Button>
