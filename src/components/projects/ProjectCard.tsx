...
export function ProjectCard({ project, onClick }: ProjectCardProps) {
  ...
  return (
    <div
      className={`bg-slate-900 border rounded-xl p-5 flex flex-col gap-4 hover:border-slate-600 transition-colors ${
        isCompleted ? 'border-slate-700 opacity-90' : 'border-slate-800'
      }`}
    >
      {/* Top row */}
      ...
      {/* Assigned to me + nearest task due */}
      <div className="flex items-center justify-between gap-2 -mt-2">
        {project.assigned_to_me > 0 && !isCompleted ? (
          <p className="text-xs text-slate-400">
            <span className="text-indigo-400 font-medium">{project.assigned_to_me}</span>
            {' '}assigned to you
          </p>
        ) : (
          <span />
        )}
        {project.nearest_task_due && !isCompleted && (
          <p className="flex items-center gap-1 text-xs text-amber-400/80">
            <Clock size={10} />
            Next: {formatDueDate(project.nearest_task_due)}
          </p>
        )}
      </div>
      ...
      <div>
        <div className="flex justify-between items-center mb-1.5">
          <span className="text-xs text-slate-400">
            Project Progress{' '}
            <span className="text-slate-500" title="Project progress = completed project tasks ÷ total project tasks × 100.">ⓘ</span>
          </span>
  ...
  const accentCls =
    accent === 'emerald'
      ...
      : accent === 'indigo'
        ...
        : 'text-white'
  return (
    <div className="bg-slate-800 rounded-lg px-2.5 py-2 flex flex-col items-start">
